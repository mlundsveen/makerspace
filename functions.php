...
function add_google_fonts() {
  wp_enqueue_style( 'wpb-google-fonts', 'https://fonts.googleapis.com/css?family=Source+Sans+Pro:200', false );
}
...
add_image_size( 'inventory_img', 360, 360 ); // 220 pixels wide by 180 pixels tall, soft proportional crop mode
add_image_size( 'front_imgs', 277, 171, true );
add_image_size( 'projects_imgs', 397, 197, true );
add_image_size( 'inventory_imgs', 40, 40, true );
add_image_size( 'employee_imgs', 250, 400, true );
...
function ms_front_events_widgets_init() {

 register_sidebar( array(
       'name' => __( 'Frontpage Events', 'matt2015' ),
       'id' => 'front-events',
       'description' => __( 'Widget on the front page, reserved for calendar', 'matt2015' ),
       'before_title' => '<h2 class="calendar-front-header">',
       'after_title' => '</h2>',
   'before_widget' => '<div id="%1$s" class="widget %2$s">',
   'after_widget' => '</div>',
   ) );
...
add_action( 'widgets_init', 'ms_front_events_widgets_init');
